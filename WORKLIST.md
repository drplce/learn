# Acorn — work list

Living checklist. Codes are stable references.
Status: ⬜ todo · 🔨 in progress · ✅ done · ⏸ gated · 🔬 research running.

## Decisions (locked)
- **D-a** (→LAY-1): text-size cap sized to **12 letters**; words 14+ shrink as a fallback. Favour bigger text for the majority.
- **D-b** (→LAY-2): on the verdict screen at biggest text, **shed harder — the continue button is sacred**; review presentation after.
- **D-c** (→WIN-5): success = **word turns green + a very subtle size pulse**.

## NOW — fix reported + protect data
- ✅ **SCR-1 / scroll drag** — **fixed on device (13.9).** Three tries first: 13.6 body-pin, 13.8 lock `main` (both failed on device). Root cause: **iOS pans the VISUAL viewport with the keyboard up** — not a DOM scroll, unstoppable by overflow/position (David's tell: the map screen, no keyboard, never moved). **13.9 (LAY-2b)** gated `touchmove` preventDefault blocks any drag not inside a scrollable region. **David confirmed: "The no scroll is working!"** Guarded by `touch-drag-gate.spec.js`.
- ✅ **SCR-2 / stuck pan alignment (13.11, LAY-2c)** — the scroll-lock leftover David then saw: on first keyboard-up iOS pans the visual viewport to the caret before the reserved band settles, and with the body pinned the pan sticks (box shoved off the top until a keyboard hide/show). `fitViewport` now cancels it — reads `visualViewport.offsetTop`, and only on a word flags `[data-panned]`/`--vvtop` and translates `#app` back down onto the visible region. Gated so non-word screens never move. Guarded by `viewport-pan.spec.js`. **→ verify on phone: does the box hold its place now, first keyboard-up and after hide/show?**
- ✅ **DAT-1** — `navigator.storage.persist()` on first tap (shipped 13.6).
- ✅ **RES-1** — Measured. On the SE a 12-letter word maxes at **~34px** (width-limited; the `--t-3xl` ceiling is never reached), 14-letter ~29px. Corrects my earlier ~44px guess. Short words currently render 51–64px, so a locked size is a real reduction — **needs D-a refined** (see below) before LAY-1.

## NEXT — deterministic layout
- ✅ **LAY-1** — **Option A shipped (13.7).** One locked ~34px word across trace/write/check; all ≤12-letter words identical (cat = presentation = 33.8px), 13+ shrink uniformly. Write box now on the same clamp (was fixed --t-2xl) and matched to .06em spacing. **Review on phone** — short words are smaller than before (the trade you accepted).
- ✅ **LAY-2** — **Locked `main` on the word screen (13.8).** Measured 0px overflow at 1.5× on the SE with the keyboard up (look/write/check), and the button lives in #act below main, so nothing clips — iOS simply can't rubber-band a `hidden` region. Grown-ups/finished keep their scroll. Guarded by `tests/word-screen-lock.spec.js`. **→ verify the drag is gone on your phone.** (The 13.3 anchor already pins the word across stages, so no separate hard-pin was needed.)
- ⬜ **DAT-2** — Surface export/import backup prominently on the grown-ups screen.

## THEN — UX batch
- ✅ **WIN-1 / miss → re-trace** — **Shipped (13.22).** A miss no longer holds on a marked word with a chevron — it **re-traces**: the correct word drops back into its box (`.tracew`), the slipped letter(s) shown **orange** (`.slip`, the slip colour winning even at the current letter so it's never hidden behind the green "type this" cue), and she copies it letter-by-letter (the existing trace mechanic — only the correct next letter lands). Completing the copy carries her on; **corrective, no re-score** (the miss was already recorded + requeued for a real from-memory go later). David's two calls (AskUserQuestion): **copy letter-by-letter** + **just move on**. Verdict steer kept in the caption slot; the "You'll see this again" note gone (the interaction is the reassurance). Removed with it: the marked word (`.marked`), the chevron (`#next`, `.primary`/`.iconbtn`/CHEV/TICK — all now dead, cleaned out), the miss note. Enter is swallowed on a re-trace (copying is the way on); it still skips the wait on a win. `W.retrace` drives the render (survives resizes) and is exposed on `session()`. ~13 test files + break.js migrated off the marked word to the re-trace. **13.25: caption ditched + escape = copy the whole word** (David). The visible verdict caption is gone from the re-trace — the orange letter + the word reappearing in its box + the spoken word-in-parts are the whole signal; the verdict wording stays on the live region (`announce`), so the mistake-reading (close/extra/apostrophe/plural, kind, never blame) is untouched and its tests read `#say` now. No skip added — copying the whole word is the way on, as David asked. Removed dead `.verdict.again` CSS. ~9 more spec files + 3 break.js sections migrated off the visible caption to the live region / slip. **→ verify on phone.** [batch 8]
- ✅ **SYL-1 / syllabifier — research, don't calculate** — **Shipped (13.23).** David: "don't try [to] calculate syllables, just research each word", and **both** (fill gaps + build the broader dictionary). Audited the letter-rule (`syllables()`) over ~130 common Year 3-6 words: it's wrong on ~50 — over-splits a vowel it should keep ("e·ve·ry·bo·dy", "dif·fe·rent", "pe·o·ple", "de·li·ci·ous") and under-splits -ible/-le/science ("terr·ible", "poss·ible", "sci·ence" as one). Fix: **`every·body`/`every·day` added to COMPOUNDS**, and a **`SYLLABLES` research dictionary** (~62 dictionary-checked splits) consulted **before** COMPOUNDS and the rule. The rule stays as the last resort for truly-unknown words; the built-in lists keep their hand-splits. Fixes the seams AND the spoken pieces (both read `splitOf`). Every entry verified to spell its word + to be the split `syllables()` actually returns (`syllables-quality.spec.js`). **13.24: second batch** — `SYLLABLES` now **147 dictionary-checked words** (added ~85 more: fash·ion, o·pin·ion, na·tion·al, pre·cious, dis·ap·pear, prob·lem, de·vel·op, cam·er·a, med·i·cine, straight·en, light·ning, and the rest of the common Year 3-6 list the rule mis-split). Corrected `reg·u·lar` (the u is its own syllable — moved it off the "gu is one sound" rule into the dictionary). Still not every word — the rule remains the last resort — but it now covers the great majority of what a school list holds. Playwright 573, sim + break green.
- ✅ **WIN-2** — Pebble-echo box shipped. 13.10: write box (seeded hand-drawn accent-green hairline via `boxHairPath()`+`blobPath`, non-scaling stroke, behind a transparent input). 13.12: **the same box now frames the trace too** (decision: box = "where you write"; the trace is a writing moment). Same seed 'box' so trace→write is one box; centre holds 338–340px across trace/write/check; the lit-green letter rides on top (David: the two greens read clean). Vertical padding only (the letters keep full width so long words don't wrap), shed on `data-tight`. Guarded by `tests/trace-box.spec.js`. [batch 4]
- ✅ **WIN-2b / syllable seams** — **Shipped (13.20).** Ditched the green `.syl` chips (David: "the sketchy organic underline of the syllable is better. Very thin hairline"). The word now stays whole and book-natural; its split is a **faint seeded hand-drawn hairline under each chunk** (`--line-2` faint ink, David's pick), on **both** the trace (in the box) and the check/win word. Built as a **measured overlay** (`drawSeams()` over `.sylseg` chunk boxes) — NOT baked into the markup, so the word's `innerText` stays a clean run (the whole harness reads words off `#screen.innerText`) and a screen reader keeps the one `role=img` spellout label. `sylRanges` only draws when the split tiles the real word and `w.length ≤ 16` (a pathological stress word renders plain + breakable). **Consequence:** the chips were tappable to hear a piece; the seams are not (a chunk is under the 44px target). Whole-word tap still replays; **a miss still speaks the word then its pieces** (`sayParts`), so per-chunk audio survives on the screen that matters. **Removed the entire two-rung `data-squeezed` machinery** (it existed only to stop the chips wrapping — seams don't wrap) and dropped `.syls` from `SHED_ORDER`. ~14 test files migrated off the chips. **13.21: seam geometry** — David: "beautiful right, everybody 5 separate lines seems wrong; make the line the same width as the letters, with a clear break." Fix: fixed inset (`min(fs·0.16, w·0.24)` each end) → a **consistent ~0.32em gap** between every pair (was a proportional gap, 11–22px, so wide words read as one line and narrow ones as dashes), seam now spans ~90% of each syllable's letters. Verified by measurement: seams sit inside their syllable's letter box, gap 11.8px uniform. **13.31: seams DITCHED altogether** (David, on device: "Let's ditch the syllable underlining altogether. Still not getting it right and wrong is worse than not at all"). The syllabifier was still splitting some words wrong on screen — `to·ge·ther` for `to·geth·er` was the last straw — and a wrong break drawn under the word is worse than none for a child learning to segment. Removed `drawSeams`, `sylSeamPath`, `sylRanges`, the `.seams`/`.sylseg` machinery and the cramped-hide rule; `sylWord` now renders the word as one clean run of letters (the slip-marking callback for WIN-1 is untouched). The word is shown WHOLE everywhere. ~10 test files migrated off the seams to "the word is shown whole, no split drawn under it" guards; teeth left in the new negative assertions. **13.33: the SPOKEN word-in-parts dropped too** (David: "Drop it too" — it shares the same imperfect splits, and a wrong break is worse than none in the ear as much as on screen). A miss now re-says the WHOLE word through `say()` (respecting read-aloud-off), never its pieces. Removed `sayParts`/`sayPartsSpoken`/`sylRate` and the dead `[data-syl]` tap-to-hear handler. Migrated the speech/audio-live tests that asserted the piece-chain to "a miss re-says just the whole word". **`splitOf`/`SYLLABLES`/`syllables()` are now VESTIGIAL** — used only by the clip prefetch (which warms part-clips that are never played) and the model-level research tests (`research`/`what-she-hears`/`syllables-quality`/`spelling`). Kept deliberately (David's SYL-1 research, and to avoid destabilising the flaky audio-live prefetch tests); a future tidy pass could remove the splitter and the part-prefetch entirely if he wants it fully gone. **WIN-2b closed as removed.**
- 🔨 **WIN-3** — Per-word pebbles done (each word its own blob, current one enlarged). **13.15: blobs centred by bbox**. **13.16–13.17: single word-screen skeleton** (dots, reserved caption slot, constant-footprint word). **13.18: pebbles must SHOW** (David: "not always showing up"). Root cause: the pebbles were hidden by `data-tight` (`--vh < 22em`) — which at large text + keyboard up (this app's core user) hits constantly — and 13.17's reserved footer had tipped the writing screen into overflow, shedding them first. Fix: footer no longer reserved (accept the miss word sitting a touch higher — a win/miss are different words); dots no longer hidden on `data-tight` (they fit, and the header going makes room); dots protected in the shed order. Measured: dots show at every text scale (1.0–1.5) and keyboard height. Guarded by `word-screen-pin.spec.js` (keyboard-up) + `spelling.spec.js` shed test (tight/cramped). **13.26: two-tone depth shipped** — presented the four depth treatments as a rendered artifact (cream + dark grounds, real seeded blobs, an "at her size" ~20px strip); David picked **B (two-tone)**. A collected pebble is now two concentric blobs (same seed): a deep-rim base (`--acc-deep`) + a smaller lit dome offset up-left (`--acc-lite`) — a raised pebble, not a flat blob; the current one (scale 1.5) is the focal one, unmet dots stay flat faint. Base path stays first so the map-shape and alignment tests still read the outline. **13.27: dome = second round** (David: "the infill should only complete in the second round through"). **13.28: enlarged pebble follows the current word** (David: "whenever a word is in focus its pebble should be larger"). The "now" pebble was keyed off *how many words were done* (`dot[done]`), so a miss — which moves her forward without finishing anything — left the big pebble one behind on the word she'd just left. Now the state is by WORD (`dw === word()` for now, `W.done` for on), so the enlarged pebble tracks whatever she is actually spelling, and jumps *back* onto an earlier word's own pebble when that word requeues. Guarded by `pebble-align.spec.js` ("the enlarged pebble follows the word she is on, even after a miss"), teeth-checked. **13.29: dome = second time in the SAME sitting** (David's clarification: "for when a word is completed for the second time in the session" — NOT a second separate day). Replaced the mastery-box gate (`box >= 3`, which made review words arrive pre-domed) with a session-scoped count: `W.corr[word]` tallies correct goes this sitting, and the lit dome shows at `corr >= 2` — the from-memory requeue she then nails, or any word she gets right twice tonight. So the dome is *earned inside the sitting*, not carried in. **13.30: pebble greens on SUCCESS, not on arrival** (David: "Pebble goes green on success"). The word she is ON stays the faint blob (just enlarged, an empty pebble waiting) until she spells it right, then it fills — so `.now` no longer takes the accent fill; only `.on` (a word added to `W.done`, which happens on a correct go) does. "Where she is" and "what she has done" are now different colours. Guarded by `pebble-align.spec.js` ("the pebble she is on stays faint until she spells it — it greens on success"), teeth-checked. **13.32: the fill is a proper reward animation** (David: "one of the main reward features so the animation needs to be carefully considered… Large grey when starting, on success pebble remains large and fills green, then shrinks as next pebble grows as it comes into focus"). Made SIZE and FILL orthogonal states (`now`=large/current, `on`=green/won — a pebble can be both), so a just-won word greens WHILE still large. The row is rebuilt from an HTML string every render (CSS transitions can't cross that), so `syncPebbles()` drives the motion via the Web Animations API, diffing each render against the last so it fires only on a real change (never on an incidental re-render — typing doesn't re-render anyway). Two moments: (1) on success the base tweens grey→green with one gentle swell, staying large (~520ms); (2) on advance the leaver tweens 1.5→1 while the newcomer tweens 1→1.5 (~320ms) — the big pebble hands off. A second-time win fades the lit dome in. Reduced motion shows the end state. Guarded by `pebble-align.spec.js` ("on a win the pebble is large AND green at once, then hands the size off to the next word"), teeth-checked against the old exclusive `on`/`now` logic. **13.34: timing review** (David: "Review the timing of the next word and animations… all need a fraction of a tweak"). Pulled every duration of the win→next flow into one tunable `REWARD` object and tightened it: fill 520→440 (brisker pop), swell 560→440 peak 1.6→1.58, the win hold (auto-advance) 1400→1150 (less dead air after the fill), handoff 320→300, and a NEW `wordIn:240` so the incoming word rises gently into focus instead of popping (WAAPI on `.tracew`/`.word` when the current word changes). Built David an interactive timing playground (Artifact) that replays the real reward with live sliders for each value, so he can dial the exact feel and read off the numbers. A pebble now reads in stages within one evening: faint outline (unmet) → **faint + enlarged** (the word she is on) → flat accent green (spelled right once) → deep rim + lit dome (spelled right again from memory). Guarded also by `pebble-align.spec.js` ("the lit dome fills only on the second time through, in the same sitting") + shed-order rooted sweep, all teeth-checked. **→ verify on phone.** ✅ WIN-3 done. [batch 1]
- ✅ **WIN-4** — **Shipped (13.14).** Speaker button and the "Listen, then write it." line gone from every word screen. She hears the word on arrival; a **tap anywhere** on the word screen (not the keyboard) replays it (`say(word,{force})`, touch-only). If she stalls ~8s on the writing stage, a **bare speaker glyph** (no box) fades in centred below the box (David's B choice), and fades out the moment she types/taps. AT preserved: the live-region announcements are unchanged, and the winning word is now focusable so keyboard focus is never dropped. Guarded by `tests/declutter.spec.js` (fails on 13.13). ~20 harness assertions that keyed off the old speaker/instruction migrated to the tap-to-hear model. [batch 2+3]
- ✅ **WIN-5** — **Shipped (13.12, Option A "Settle").** The every-time "Yes — that's it." caption is gone; a correct word turns accent green and breathes once (`@keyframes acorn-settle`, `--m-quick`, folded colour so it fires on insert, reduced-motion safe). The earned "You got there." stays after a fought-back miss. `id="advance"` moved onto the winning word (tap still skips the wait); the spoken verdict is untouched (a11y). Guarded by `tests/win-cue.spec.js`. [D-c, RES-2]

## LATER — infra + gated pacing
- ✅ **RES-2** — Success-moment design ready (feeds WIN-5): Option A "Settle" — word greens 300ms + one-shot 1.0→1.04→1.0 pulse (420ms, settle-ease), reduced-motion safe. Build note: move `id="advance"` onto the word. `reference/success-moment-design.md`.
- ⬜ **INF-1** — Service worker for guaranteed offline launch (skipWaiting + network-first HTML + bumped VERSION).
- ⬜ **INF-2** — Manifest + icons (192/512) + `format-detection` meta.
- ✅ **RES-3** — Pacing analysis done: both breaches are **30-day-window measurement artifacts; the engine is correct.** tionsion is learned (98.9% by 60d), aussie's 92.5% is a finished-list revision tail. `reference/pacing-analysis.md`.
- ✅ **PACE-1** — **B2 + B3 shipped (owner-approved).** sim.js bands recalibrated (learned floor 0.80→0.75; finished-list exemption 0.5→3), engine untouched. **sim now green.**

## Done / closed
- ✅ **HAR-4 (2026-09-11)** — §6 item 5 (real input), the least-recently-exercised rotation item.
  Typing, Enter, paste refusal and dictation detection were all well covered; **autocorrect was
  covered at one stage of three.** `#type` is rendered at three separate places — the trace, the
  write, and the re-trace — each with its own attribute list, and the test only read the write one.
  All three are correct today (checked, not assumed); the test now reads all three, teeth-checked by
  dropping `autocorrect="off"` from each of the two that were unguarded. This is the highest-stakes
  invisible defect the app could have: if iOS fixes her spelling on the way in she is marked right
  for a word she got wrong, the box climbs, and the screen looks identical. The grown-ups' paste box
  has no `autocorrect` attribute, which is fine — that is a parent pasting a word list, not her
  spelling, and capitalisation and spellcheck are off there anyway.
- ✅ **HAR-3 (2026-09-03)** — the cadence gate gained a **sync-first step** (a reclaimed container can
  come up on an older snapshot, and the STATE block then lies about when the last pass ran), plus the
  **capture-the-output** practice and the standing lesson that an intermittent failure is a test
  defect until proven otherwise. Both carried across from the sibling app, where a `| grep | tail`
  hid an intermittent failure for a fortnight and three captured runs then found it was **two**
  unrelated flaky tests. Applied the same three-run technique to Acorn: **590 passed on all three**,
  break.js 60 checks clean, sim in band — so Acorn's suite is stable, and 144's flakiness was
  specific to its newer timing-dependent tests rather than a property of the shared harness style.
  *(Also caught mid-pass: I first ran break.js concurrently with the suite, which this project knows
  can contaminate `file://` localStorage between runs. Re-ran it alone before trusting it.)*
- ✅ **HAR-5 (2026-09-23)** — two new things measured, no defect found. (1) **The suite runs under
  daylight saving now.** Every run it has ever had was under UTC, while the engine measures
  intervals in days across LOCAL midnights; `TZ=Australia/Sydney npx playwright test` → 594 pass.
  Written into ROUTINE.md §5 as an occasional practice beside `ACORN_DAYS_AHEAD`. (2) **The
  two-window guard on a grown-up's LISTS is the merge, not the `storage` listener** — 40 trials of
  a list pasted in one window while she answers in the other, ten with her listener deliberately
  removed, none lost the list. Extends the August measurement from her answers to his lists.
  ⬜ **Open, deliberately not closed:** the documented `two-windows` flake is still a flake —
  ~1 failure in 9 full runs, 12/12 in isolation, and 40 instrumented trials could not reproduce it.
  ROUTINE.md records what was ruled out so the next attempt starts further along instead of
  repeating them.
- ✅ **COPY-4 (2026-09-22)** — the voice paragraph described a phone it had never looked at, and
  the §6.2 sweep is now finished. It announced the list as "Australian first" and recommended
  "Karen is usually the best it can do" unconditionally; `offerVoices()` only puts en-AU first when
  the phone HAS one. Stubbed a UK iPhone and a laptop with a single American voice: "**offering 1
  English voice, Australian first**", Karen recommended and absent. Not exotic — the grown-ups
  screen is the one a parent opens on a laptop to paste a list. Both clauses now read the list; the
  replacement avoids its own agreement slip ("none of them" is plural against "1 English voice", so
  "and none is"). `speech.spec.js` "the voice paragraph describes the phone it is actually on"
  drives four stubbed phones and checks the fixture took before trusting it. Teeth-checked. No
  VERSION bump.
  **Checked and left alone, with reasons:** the saved-lists drip hint ("a couple of words at a
  time" is vague prose, and "never dumped on her at once" holds at every lever setting) and the
  backup summary (every count already guarded). **The §6.2 named list is exhausted — the question
  stays as something to ask of new copy, not a backlog.**
  *Also corrected: COPY-3's note said newMax 0 gives her zero new words. True only with review
  material — on an empty slate `buildSession`'s MIN_SESSION floor still hands her one, deliberately.*
- ✅ **COPY-3 (2026-09-21)** — the pace paragraph recited the defaults of the steppers below it.
  "Above 85% … up to three new words, in the middle eight words and one new, under 70% six words and
  nothing new" is 3 / 1 / 0, which are the DEFAULTS of `newMax`/`newMin` — two grown-up levers a few
  inches down the same screen. At newMin 2 / newMax 5 the engine hands her **5 / 2 / 2** and the
  paragraph still said three / one / nothing, while OPENING with `paceLabel()`'s "5 new words a
  session" — self-contradicting inside one paragraph. "New words never take more than a third of a
  sitting" was false there too (5 of 9). Bands now read the levers through the same `clampN` the
  engine uses; the third-of-a-sitting line states what is always true (the ceiling bounds it) with
  the third given as what the suggested 3 works out to. Checked first that the engine was right:
  **at newMax 0 she really does meet zero new words**, and sim numbers are byte-identical, so no
  behaviour moved. `pace-settings.spec.js` "the pace paragraph says what the engine will actually
  do" drives six lever settings × three bands, compares against the ENGINE rather than a fixed
  string, and also requires the paragraph to name **no** count the engine will never give — which is
  how the old copy failed. Teeth-checked. No VERSION bump — grown-ups screen.
  **Third consecutive pass to find a sentence falsified by a setting, a scheme or a count of one;
  `ROUTINE.md` §6.2 now carries this as a standing sweep rather than a lucky catch.**
- ✅ **COPY-2 (2026-09-20)** — the map caption was true on one screen of two. "They fill in and
  darken as she learns them" describes light mode; a cell is `--acc` over `--surface`, and those
  swap places at night, so learning a word runs L 0.713 → 0.131 on cream and L 0.047 → **0.446** on
  the dark screen. A grown-up reading at night was told the learned words are the dark ones while
  looking at a picture whose learned words are the bright ones. Reworded in the direction true on
  both — "fill in and grow stronger", "fades rather than disappearing" — rather than made
  conditional on the scheme, which can change under a live page; the ramp's code comment said the
  same one-scheme thing and was corrected with it. `network.spec.js` "the caption is true on the
  dark screen as well as the light one" measures the direction in each scheme, asserts the two
  really do run opposite ways, and bans any one-scheme direction word, so swapping "darken" for
  "lighten" does not satisfy it. Teeth-checked both ways. No VERSION bump — grown-ups screen.
  **Found by §6.1's screenshot-and-look, not by the suite:** all 590 tests ran in the default
  scheme, where the sentence was true.
- ✅ **COPY-1 (2026-09-19)** — the grown-ups map caption had no singular. At exactly one faint
  outline it still read "the few she is coming to next as faint outlines", which is the same defect
  the caption had already been fixed for twice (promising hairlines with none drawn, promising
  outlines with none drawn) one rung down. `ahead === 1` is reachable at the end of a short pasted
  list or the end of everything she has. Given a singular form; the neighbouring "the rest of her
  lists" clause left alone on purpose (the horizon walk breaks after one untouched family, so one
  outline can legitimately coexist with undrawn lists). `network.spec.js` "one outline is one
  outline, not a few" checks both directions and was teeth-checked both ways. No VERSION bump — a
  grown-up screen, so she would not notice. Found by a new agreement sweep over every readable
  phrase (text, `aria-label`s, placeholders, `#say`) in five count-sensitive states; **the sweep's
  first version was lying** — it split `innerText` on newlines and a stat tile is
  `<b>1</b><span>sittings</span>`, so it reported clean with the plural guard deleted. Rebuilt per
  element and re-teeth-checked. On the real build every other count-bearing string is already
  guarded.
- ✅ **HAR-2 (2026-08-26)** — `ACORN_DAYS_AHEAD=N` runs the whole suite at a simulated future date
  (off by default). Added after the sibling app's engine tests rotted on the calendar and went red
  on an untouched build; Acorn proved clean at +200 and +1100 days. Also measured, and written into
  ROUTINE.md §5: the two-window data guard is the `rev` merge, **not** the `storage` listener —
  `break.js` §31 still passes with the listener stubbed out.
- ✅ **HAR-1 (2026-08-18)** — the adversarial pass gained three sections about the **re-trace**, the
  newest mechanic and the one she meets on every slip: it cannot be escaped by wrong keys or stray
  keys, cannot be pasted through, and never re-scores the word (§37); an evening of nothing but
  misses still ends rather than requeueing forever (§38); a reload mid-copy loses nothing and counts
  one slip once (§39). All teeth-checked. Also: `__acorn.reset()` now clears the faked test clock —
  latent here (every caller sets the date again first, and sim.js output is byte-identical either
  way) but a live trap in the sibling app, so guarded in `engine.spec.js` rather than left to luck.
  `session()` now exposes `traced`, without which §37 could not measure anything.
- ✅ 13.1 trace letter green · 13.2 scroll-lock v1 · 13.3 anchor · 13.4 drop write-instruction · 13.5 mic-on-trace fix
- ✅ CLOSED: iOS keyboard accessory bar — impossible to remove from any web app (research-confirmed); accepted.
- ✅ `reference/home-screen-webapp.md` — Apple home-screen web-app research.
