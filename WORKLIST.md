...
- 🔨 **WIN-3** — Per-word pebbles done (each word its own blob, current one enlarged). **13.15: pebbles locked vertically** — each blob centred by its own bounding box (was drawn at a jittered/organic-offset point, which the current dot's 1.5 scale magnified into a visible "drop" when it filled green — David's report). Row is now dead level; guarded by `tests/pebble-align.spec.js`. (Two-pass green depth for twice-cycled words still open.) [batch 1]
