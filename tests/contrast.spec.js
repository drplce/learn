    ...
    for (const tint of TINTS) {
      test(`${tint}: every text node clears AA`, async ({page}) => {
        ...
        for (const [screen, stage] of [['day','look'], ['day','write'], ['day','nearly'],
          ...
          // done = the finished screen is fully wordless now (David) — the line and the praise are
          // spoken, not drawn, and the way on became a breathing pebble with no words — so there is
          // no text on it at all to measure. (The dead end, which is words not a picture, still has
          // its two lines.) The ratio filter below still catches any text that IS there.
          expect(rows.length, `${screen}/${stage} rendered nothing`)
            .toBeGreaterThanOrEqual(stage === 'deadend' ? 2 : stage === 'write' ? 0
                                  : stage === 'cued' ? 2 : stage === 'done' ? 0 : 3);
